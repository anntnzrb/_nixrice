...
let
  ...
  # Everything built from one module tree ({ base, features, profiles } roots):
  # the flake modules per class and the machine wiring on top of them.
  load =
    roots:
    let
    ...
    in
    {
      ...
      # What clan.nix gives a machine: its class's base, the profile named
      # after each of its tags (modules/profiles/<tag>), and its home.nix, if
      # any, as the fleet user's Home Manager configuration on top of the
      # home base.
