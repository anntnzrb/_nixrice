...
do
    ...
    awscreen.connect_for_each_screen(function(s)
        awtag({ "𝛼", "ϐ", "ℽ", "𝛿", "ε", "ϝ", "ͷ", "ϛ", "𝜁" },
            s, awful.layout.layouts[1])

        -- layoutbox
        local layoutbox = awwidget.layoutbox(s)
        layoutbox:buttons(gears.table.join(
            awbutton({}, mbuttons.left,
                function() awful.layout.inc(1) end),
            awbutton({}, mbuttons.right,
                function() awful.layout.inc(-1) end)
        ))
        ...
        local taglist = awwidget.taglist({
            ...
            -- focus tag
                ...
                awbutton({ keys.super }, mbuttons.middle,
                    function(t)
                ...
                awbutton({ keys.super }, mbuttons.right,
                    function(t)
...
do
    ...
    local clientbuttons = gears.table.join(
        ...
        awbutton({ keys.super }, mbuttons.left,
            function(c)
                c:emit_signal("request::activate", "mouse_click",
                    { raise = true })
    ...
    -- awesome
        awkey({ keys.super, keys.meta }, "r",
            awesome.restart,
            {
                description = "reload awesome",
                group = "awesome"
            }),
        awkey({ keys.super, keys.meta }, "q",
            awesome.quit,
            {
        ...
        -- prompts
        awkey({ keys.super }, "r",
            function() awscreen.focused().mypromptbox:run() end,
            {
    ...
    -- focus/move/swap
        awkey({ keys.super }, "j",
            function() awclient.focus.byidx(1) end,
            {
        ...
        awkey({ keys.super, "Shift" }, "j",
            function() awclient.swap.byidx(1) end,
            {
                description = "swap with next client",
                group = "client"
            }),
        awkey({ keys.super, "Shift" }, "k",
            function() awclient.swap.byidx(-1) end,
            {
        ...
        -- resizing
        awkey({ keys.super }, "l",
            function() awtag.incmwfact(0.01) end,
            {
        ...
        -- screens
        awkey({ keys.super, "Shift" }, "bracketleft",
            function() awscreen.focus_relative(1) end,
            {
                description = "focus next screen",
                group = "screen"
            }),
        awkey({ keys.super, "Shift" }, "bracketright",
            function() awscreen.focus_relative(-1) end,
            {
                description = "focus prev. screen",
                group = "screen"
            }),
        awkey({ keys.super, "Shift" }, "backslash",
        ...
        -- misc
        awkey({ keys.super, "Shift" }, "q",
            function(c) c:kill() end,
            {
                description = "close",
                group = "client"
            }),
        awkey({ keys.super, "Shift" }, "f",
            function(c)
        ...
        awkey({ keys.super, "Control" }, "space",
    ...
    for i = 1, 9 do
        globalkeys = gears.table.join(
            ...
            -- toggle tag view
            awkey({ keys.super, "Control" }, "#" .. i + 9,
                function()
                    local tag = awscreen.focused().tags[i]
                    if tag then awtag.viewtoggle(tag) end
                end,
                {
            ...
            awkey({ keys.super, "Control", "Shift" }, "#" .. i + 9,
                function()
                    if client.focus then
                        local tag = client.focus.screen.tags[i]
...
do
    ...
    -- focus follows mouse
    csignal_connect("mouse::enter",
        function(c)
            c:emit_signal("request::activate", "mouse_enter",
                { raise = false })
        end)

    csignal_connect("focus",
        function(c)
            c.border_color = beautiful.border_focus
        end)

    csignal_connect("unfocus",
        function(c)
